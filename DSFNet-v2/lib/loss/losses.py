...
class IOULoss(nn.Module):
    ...
    # def forward(self, output, mask, ind, target):#output['reg'], batch['reg_mask'],batch['ind'], batch['reg']
        # pred = _transpose_and_gather_feat(output, ind)
        # mask = mask.unsqueeze(2).expand_as(pred).float()
        # loss = F.iou(pred * mask, target * mask, reduction='elementwise_mean')
    def forward(self,output, target):
        ...
        pred = _transpose_and_gather_feat(output['boxes'], target['ind'])
        #TODOoutput['boxes']
        ...
        c_y_t = torch.div(centers, img_w[:, None], rounding_mode='trunc')
        pred[:, :, 0] += c_x_t
        pred[:, :, 1] += c_y_t
        ...
        # pred_boxes=torch.cat([target['reg'], target['wh']], dim=1)
        collect_pred = pred.view(-1, 4)[mask.view(-1) == 1, :]
        collect_tgt = target['gt_det'].clone().view(-1, 4)[mask.view(-1) == 1, :]
        ...
        if collect_tgt.shape[0] > 0:
            ...
            pred *= scale_fct[:, None, :]
            target_boxes_unnorm = target['gt_det'].clone() * scale_fct[:, None, :]
